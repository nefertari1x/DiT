...
from qdt_data import NpyDepthCrossDataset
...
def main(args):
    ...
    dataset = NpyDepthCrossDataset(
        filelist_path=args.filelist,
        orig_image_size=args.orig_image_size,
        sqr_grid_size=sqr_grid_size,
        num_depth_levels=args.num_depth_levels,
    )
    ...
    debug_log_dir = f"{experiment_dir}/debug" if rank == 0 else "/tmp/debug_dummy"
    monitor = GradientMonitor(model, rank=rank, log_dir=debug_log_dir)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, depth_seq, depth_centers, y in loader:
            x = x.to(device)
            depth_seq = depth_seq.to(device)
            depth_centers = depth_centers.to(device)
            y = y.to(device)
            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
            model_kwargs = dict(y=y, depth_seq=depth_seq, depth_centers=depth_centers)
            ...
            # DEBUG: 检测异常并落盘现场，但不跳步（DDP collective 一致性 + 靠 clip_grad 兜底）
            ...
            if train_steps % args.log_every == 0:
                # DEBUG: 定期记录每层统计量到 TensorBoard
                monitor.log_periodic(train_steps, tb_writer)
